"""Evaluate and register the Phase 10 causal turnover-control grid."""
...
from src.postprocess.phase10 import run_turnover_grid
...
def main() -> int:
    ...
    report = run_turnover_grid(config, overwrite=args.overwrite)
    compact = {
        "selected": report["selected"],
        "candidate_count": report["candidate_count"],
        "grid_rows": report["grid_outputs"]["rows"],
        "registry": report["registry"],
        "test_data_used": report["causal_contract"]["test_data_used"],
    }
    logger.info("Phase 10 selected: %s", compact["selected"])
    print(json.dumps(compact, ensure_ascii=False))
    checks = [
        report["candidate_count"] == 18,
        report["grid_outputs"]["rows"] == 72,
